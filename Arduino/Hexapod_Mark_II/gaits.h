...
/* Transition time should be of the form (k*BIOLOID_FRAME_LENGTH)-1
...
ik_req_t DefaultGaitGen(int leg){
  if( MOVING ){
    ...
    else if(((stepNumber == gaitLegNo[leg]+1) || (stepNumber == gaitLegNo[leg]-(stepsInCycle-1))) && (gaits[leg].z < 0)){
...
ik_req_t SmoothGaitGen(int leg){
  if( MOVING ){
    ...
    else if((stepNumber == gaitLegNo[leg]+1) && (gaits[leg].z < 0)){
      // leg up position
      gaits[leg].x = 0;
      gaits[leg].y = 0;
      gaits[leg].z = -liftHeight;
    ...
    else if((stepNumber == gaitLegNo[leg] + 2) && (gaits[leg].z < 0)){
      // leg halfway down                                
    ...
    else if((stepNumber == gaitLegNo[leg]+3) && (gaits[leg].z < 0)){
      // leg down position                                           NOTE: dutyFactor = pushSteps/StepsInCycle
      gaits[leg].x = (Xspeed*cycleTime*pushSteps)/(2*stepsInCycle);     // travel/Cycle = speed*cycleTime
      gaits[leg].y = (Yspeed*cycleTime*pushSteps)/(2*stepsInCycle);     // Stride = travel/Cycle * dutyFactor
      gaits[leg].z = 0;                                                 //   = speed*cycleTime*pushSteps/stepsInCycle
